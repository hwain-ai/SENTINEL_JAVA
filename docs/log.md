...
- **Fix** mutation/junit/JUnitPrivateFiles.java·src/test/.../LockedToolchain.java: 첫 4플랫폼 CI 에서 Linux x86_64·arm64 는 통과, macOS 두 러너는 같은 27개 시험이 실패했다. 원인 둘. (1) JDK 17 의 macOS 파일 시스템은 SecureDirectoryStream(디렉터리 핸들 기준 열기)을 제공하지 않아 JUnit 리스너가 이벤트 파일을 아예 쓰지 못했고(junitSecureDirectoryUnsupported), 그 뒤의 변이·PIT 탐침 시험이 "이벤트 없음" 으로 연쇄 실패했다. 핸들 경로가 없는 파일 시스템에서는 조상 폴더가 모두 링크가 아닌 실제 폴더인지 확인한 직후 CREATE_NEW·NOFOLLOW_LINKS 로 여는 경로(`writeByPath`)를 쓴다. 확인과 열기 사이의 교체만 이 경로의 남은 틈이며, Linux 시험도 이 경로를 직접 덮는다. (2) 통합 시험이 JAVA_HOME 을 `.toolchain/jdk-17.0.20.1+1` 로 고정해 macOS 묶음의 `Contents/Home` 을 못 찾았다(javaExecutableInvalid). 시험 보조 LockedToolchain 이 플랫폼에 맞는 위치를 준다.
